...
- Added an opt-in Firebase two-player room foundation: short-code creation/joining, shared seeded session state, alternating active turns, version-checked transactions, presence heartbeats, reconnect storage, 24-hour expiry gates/leave cleanup, and local-only fallback when Firebase is unavailable.
- Added synchronized room text chat with safe text rendering, a 280-character limit, connection state, unread-ready compact panels in the lobby and game screen, and mobile-responsive layout. Added Firestore rules for room membership, messages, presence, and WebRTC signaling.
- Added optional WebRTC voice scaffolding with explicit microphone consent, mute-by-default push-to-talk, speaking state, STUN configuration, optional TURN slots, signaling cleanup, and graceful text/gameplay fallback when voice is unsupported or denied.
- Multiplayer static and offline UI checks passed; live two-context Firebase synchronization, Firestore emulator/rules execution, and cross-device WebRTC/TURN QA remain pending because this checkout has no Firebase project configuration and no Firebase CLI/emulator installed.
...
- Added a four-stage floor transition using the committed `elevator-doors.jpeg` and atmospheric parallax art: `DOORS CLOSING` → `ASCENDING` → `ARRIVAL CONFIRMED` → `DOORS OPENING`, with the next floor board rendered before reopening. Transition state is included in `window.render_game_to_text`, keyboard/lifeline mutations are locked during transit, and reduced-motion uses zero-duration stage timing while retaining semantic status updates.
- Wired the supplied elevator audio clips for button, step-in, ascent, and correct-guess cues. Browser verification passed on desktop and 390x844 portrait: transition stages, floor 01→02 and 02→03 flow, key lock, final board readiness, `aria-busy`, and no document overflow all passed with no console errors. Reduced-motion verification completed the same semantic flow in under 100ms with no errors.
- Visual QA initially found the transition overlay constrained to the header by the generic game child-layer rule; excluding `#elevator-transition` fixed it. The closed-door capture now fills the game viewport and visibly uses the committed door artwork over the atmospheric scene.
- Added `assets/audio/Relay Shaft Ritual.ogg` as a looping background music bed. It starts only after a user gesture for autoplay compatibility, continues across floor transitions, and follows the existing SOUND ON/OFF control; muting stops music and active effects immediately.
- Expanded the deterministic target catalog to every available common-word answer at each floor length: 36 guessable two-letter answers on floor 02 and 535–5,008 targets per floor from 03–13. Themed entries retain custom clues whenever they overlap the common catalog; other common targets receive validated fallback clue metadata.
- Replaced runtime Web Audio oscillator synthesis with file-backed `.ogg` playback. Added dedicated Ogg/Opus cues for door close/open, boarding confirmation, letter input, present letters, invalid words, reveal, clue, 50:50, floor arrival, pass-and-play handoff, run clear, and failure; existing button, step, ascent, and correct clips remain in use. Playback now uses short-lived HTML audio instances so rapid keyboard input can overlap without resetting a shared clip, and mute stops active clips immediately.
- Added a persistent segmented floor HUD with current floor, direction, next stop, and accessible live state. The existing transition sequence now includes suspense, floor-display ticks, brake/arrival emphasis, floors 10–13 story beats, a skippable cinematic path, paused gameplay time during holds, and a stronger floor 13 finale. `render_game_to_text` now exposes HUD, suspense, cinematic, timer, and attempt-limit state.
- Replaced the definition clue lifeline with `REROUTE`: one atomic extra attempt on the current floor, shared through local, pass-and-play, and online session state. Older saved runs migrate `clue` to `reroute`; the next floor resets the temporary bonus.
- Browser QA passed for the new HUD at desktop and 390x844, reduced-motion boarding/arrival, reroute grant and one-use lockout, sixth-guess suspense with time paused and keyboard skip, floor 10 story stage, floor 13 finale cleanup, and no console or failed-request errors. New suspense/ambient OGG slots are wired to existing file-backed cues until the approved replacement files are supplied; no runtime synthesis was added.
