...
const Floor13Engine = {
  ...
  showLobby() { document.getElementById("lobby-screen").hidden = false; document.getElementById("game-screen").hidden = true; document.getElementById("game-container").scrollTop = 0; document.getElementById("resume-btn").hidden = !Floor13Storage.read(STORAGE_KEYS.active, null); Floor13UI.updateAudioButton(); Floor13UI.renderStats(); },
  startDaily() { this.startRun("DAILY", this.dailySeed()); },
  startChallenge() { const seed = this.hashSeed(`${Date.now()}-${Math.random()}`); this.startRun("CHALLENGE", seed); Floor13UI.copyChallengeLink(seed); },
  startPassPlay() { this.startRun("PASS_PLAY", this.hashSeed(`pass-${Date.now()}`), [Floor13UI.handle(), "Guest 2"]); },
  ...
  init() {
    document.addEventListener("click", event => {
      ...
      const actions = {
        ...
        retry: () => { this.closeTerminal(); Floor13Engine.startRun(Floor13Engine.mode, Floor13Engine.seed, Floor13Engine.run?.players || [this.handle()]); }, share: () => this.shareResult(), lobby: () => { this.closeAllOverlays(); Floor13Engine.showLobby(); },
