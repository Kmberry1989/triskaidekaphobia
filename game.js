...
const Floor13Engine = {
  ...
  addLetter(letter) { if (!this.run || this.run.result !== "IN_PROGRESS" || this.run.attempts >= MAX_ATTEMPTS || this.shatteredKeys.has(letter)) return; const slot = this.currentGuess.indexOf(""); if (slot === -1) return; this.currentGuess[slot] = letter; Floor13UI.clearInvalidEntry(); Floor13Audio.beep("tap"); Floor13UI.updateCurrentGuess(); },
  removeLetter() { const slot = this.currentGuess.map((letter, index) => letter ? index : -1).filter(index => index >= 0).pop(); if (slot === undefined) return; this.currentGuess[slot] = ""; Floor13UI.clearInvalidEntry(); Floor13UI.updateCurrentGuess(); },
